...
import { validateCheckoutForm, validateCart } from '../utils/checkoutValidation';
...
function Checkout() {
  ...
  const shouldRedirect = (!cartItems || cartItems.length === 0) && 
                         currentStep !== 6 && 
  ...
  const displayCartItems = (cartItems && cartItems.length > 0) ? cartItems : savedCartItems;
  const displayTotal = (cartItems && cartItems.length > 0) ? calculateTotal() : savedTotal;
    ...
    <div className="checkout-container">
      ...
      {(currentStep < 5 && currentStep !== 6) && (
        <div className="checkout-steps">
          <div className={`step ${currentStep === 1 ? 'active' : currentStep > 1 ? 'completed' : 'inactive'}`}>
          ...
          <div className={`step ${currentStep === 3 ? 'active' : currentStep > 3 ? 'completed' : 'inactive'}`}>
              ...
              return (
                <div key={item.id} className="cart-item-summary">
                  ...
                  <div className="item-details">
                    ...
                    <span className="item-quantity">{item.quantity} x {formatCurrency(itemPrice)}</span>
                  </div>
